...
`query.py` использует `logging.getLogger("generate_query")` — пишет в консоль и в `logs/debug.log` с таймстемпом. При каждом вызове в лог попадает либо полный список `tool_calls` с аргументами, либо факт прямого ответа без поиска.
...
def grade_documents(state) -> Literal["answer", "rewriter"]:
    ...
    if rewrite_count >= 2:
        return "answer"       # лимит попыток исчерпан

    if not context or len(context.strip()) < 50:
        return "rewriter"     # пустой контекст — переспросить
...
Использует **отдельный экземпляр модели** (`get_grader_model()`) с `temperature=0` — не зависит от `get_response_model()` из `query.py`. Это позволяет при необходимости заменить модель-оценщик независимо от основной модели.

Промпт `GRADE_PROMPT_STRICT` требует ответ строго `yes` или `no`. Проверка `"yes" in answer` — намеренно мягкая: модели иногда отвечают `"yes, ..."` вместо чистого `yes`.

### Логирование

`grader.py` использует `logging.getLogger("grader")` — пишет в консоль и в `logs/debug.log` с таймстемпом. В лог попадает: номер попытки, вопрос (первые 60 символов), ответ модели и принятое решение.
...
Вызывается **только когда история превышает порог** (> 50 сообщений) — решение принимается в `should_summarize` до входа в узел.  
...
Стратегия: история растёт → достигает 50 → сжимается до 4 → снова растёт → снова сжимается.
...
START → query (LLM решает: вызвать retrieve_docs)
      ...
      → answer
      → should_summarize (история ≤ 50) → END
```
...
START → query → retrieve → grade_documents → "answer"
      → answer
      → should_summarize (история > 50) → summarizer → END
```
...
Узлы `query.py` и `grader.py` используют стандартный `logging` вместо `print`. Все сообщения пишутся в **один файл** `logs/debug.log` в корне проекта и одновременно в консоль. Папка создаётся автоматически.

Паттерн одинаков в обоих файлах:
...
if not logger.handlers:
    ...
    logger.addHandler(logging.FileHandler(_LOG_PATH, encoding="utf-8"))
    logger.addHandler(logging.StreamHandler())
```

Проверка `if not logger.handlers` защищает от дублирования хендлеров при hot-reload Streamlit.

| Логгер | Что пишет |
|--------|-----------|
| `generate_query` | Какой tool вызван и с какими args, либо факт прямого ответа |
| `grader` | Номер попытки, вопрос, ответ модели, принятое решение |
